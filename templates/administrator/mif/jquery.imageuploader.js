...
/* global jQuery FormData FileReader */
   ...
   var file=$("#im"+a).data('file')
$("#im"+a).hide('slow')



var result = $("#field_gallery").val().split(',');
var rst=[]
var index2=0
...
$("#field_gallery").val(str)
           $.ajax({
                        type: 'POST',
                        url: '/sample_module/del_image/'+file,
             
                        success: function (response) {
                         alert('Файл удален и ну его нахер')
...
$("#field_gallery").val(str)
           $.ajax({
                        ...
                        success: function (response) {
                         alert('Фавйл удален и ну его нахер')
    ...
    $.fn.uploader = function (options, testMode) {
        return this.each(function (index) {
            options = $.extend({
                submitButtonCopy: 'Загрузить выбранные',
                instructionsCopy: 'Перетащите файлы сюда или',
                furtherInstructionsCopy: 'Вы можете загрузить еще файлы или',
                selectButtonCopy: 'Выберите файлы',
                secondarySelectButtonCopy: 'Еще файлы',
                dropZone: $(this),
                fileTypeWhiteList: ['jpg', 'png', 'jpeg', 'gif', 'pdf'],
                badFileTypeMessage: 'Некорректный тип файла',
                ajaxUrl: '/sample_module/upload',
                testMode: false
            }, options);
