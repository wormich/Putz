    ...
    <section class="mini-layout">
        <div class="frame_title clearfix">
            ...
            <div class="pull-right">
                <div class="d-i_b">
                    <button type="button" class="btn btn-small btn-danger disabled action_on" onclick="delete_function.deleteFunction()" id="module_delete">
                        <i class="icon-trash"></i>{lang("Delete","admin")}</button>
                    {if end(explode(' ',IMAGECMS_NUMBER)) != 'Professional' || MAINSITE}
                        <button type="button" class="btn btn-small btn-success" onclick="window.location.href = '/admin/languages/create_form'" data-submit>
                            <i class="icon-plus-sign icon-white"></i>{lang('Create language',"admin")}</button>
                    {/if}
                </div>
            </div>
        </div>
