...
$('.addVariantToCart').die('click').live('click', function () {
    //РЈСЃР»РѕРІРёРµ СѓР±СЂР°РЅРѕ РІ СЃРІСЏР·Рё СЃ Р·Р°РґР°РЅРёРµРј ICMS-1518
        ...
        $.ajax({
            ...
            complete: function (res) {
                ...
                if (i < 100) {
                    ...
                    showMessage('РћР±РЅРѕРІР»РµРЅРёРµ', 'РћР±РЅРѕРІР»РµРЅРёРµ РїСЂРѕС€Р»Рѕ СѓСЃРїРµС€РЅРѕ');
                }
            }
        });
    },
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res) {
                    ...
                    showMessage('РЎРѕРѕР±С‰РµРЅРёРµ', 'Р¤Р°Р№Р» СѓСЃРїРµС€РЅРѕ СѓРґР°Р»РµРЅ');
                } else {
                    showMessage('РћС€РёР±РєР°', 'Р¤Р°Р№Р» РЅРµ СѓРґР°Р»РµРЅ', 'r');
                }
            }
        });
    },
...
$(document).ready(function () {
    ...
    $('body').on('keyup', 'input.email', function () {
        if (/[Р°-СЏС‘С‹]/gi.test($(this).val()))
            $(this).val($(this).val().replace(/[Р°-СЏС‘С‹]/gi, ""));
    });
...
function ch (ev){
    $('.f-dock').each (function () {
    ts = $(this); 
    str=''; 
    ts.find('.new-oh').each(function () {
    i=0;
    j=0;
    arr = new Array();
    tts = $(this);
    les = tts.find('input').length;
    i++;
    //console.log(tts.find('input').first().val());
    if (tts.find('input').first().val()!=''){
    tts.find('input').each(function () {
    dds = $(this);
    j++;  
    if (j==les){
    str+=dds.val()+'##';  
    ...
    ts.find('.docinplast').val(str.slice(0, -2));
    });
};
$('.adds span').click(function () {
  var a = $(this).parents('.f-dock').find(".new-oh").first().find('.docinp').val(); 
  console.log(a);
  if (a!=''){
  var vs =  $(this).parents('.f-dock').find(".new-oh").first().clone();
  $(this).parents('.f-dock').find(".frmss").append(vs);
  $(this).parents('.f-dock').find(".new-oh").first().find('input').val('');
  } else {
  $(this).parents('.f-dock').find(".new-oh").first().find('.docinp').css('border-color', 'red'); 
  setTimeout(function () {$(".new-oh").find('.docinp').css('border-color', '')},1000);
  }; 
});
function dels (a) {
    $(a).parents('.new-oh').remove();
    $(".new-oh").first().find('.docinp').change();
};
$(document).ready(function () {
    var a = 'http://t2.artofseo.ru/uploads/pmr_code-of-conduct_ru.pdf';
    
 $('.f-dock').each(function () {   
 str = '';
   var dock=$(this).find('.docinplast').val();
   console.log(dock);
   if (dock){
   var onedock = dock.split('##');
   $(onedock).each(function () {
    var oned = $(this);
    var oned = oned[0];
    var line = oned.split('||');
    if (line[0]!=false){
str+='<div class="new-oh"><div class="group_icon pull-right"><span onclick="dels($(this))" class="btn deldock">Удалить</span></div><input onchange="ch($(this))" type="text" class="docinp" value="'+line[0]+'" /> <input onchange="ch($(this))" type="text" class="docname" value="'+line[1]+'" placeholder="Альтернативное имя"/></div>'; 
}     
   });
   };
   $(str).appendTo($(this).find(".frmss"));
  }); 
});