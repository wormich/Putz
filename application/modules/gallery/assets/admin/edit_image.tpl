<section class="mini-layout">
    <div class="frame_title clearfix">
        <div class="pull-left">
            ...
            <span class="title w-s_n">{lang("Album", 'gallery')}
                : <a href="/admin/components/cp/gallery/edit_album/{$album.id}" class="">{$album.name}</a></span>
        </div>
        <div class="pull-right">
            <div class="d-i_b">
                <a href="/admin/components/cp/gallery/edit_album/{$album.id}" class="t-d_n m-r_15 pjax"><span class="f-s_14">←</span>
                    <span class="t-d_u">{lang('Back', 'admin')}</span></a>
                <button type="submit" class="btn btn-primary formSubmit" data-form="#change_img_desc">
                    <i class="icon-ok"></i> {lang("Save", 'gallery')}</button>
                ...
                <div class="row-fluid inside_padd">
                    ...
                    <div class="span7">
                        <dl class="dl-horizontal m-t_20">
                            <dt>{lang("Image file name", 'gallery')}:</dt>
                        ...
                        <form method="post" action="{site_url('admin/components/run/gallery/update_info/' . $image.id . '/' . $locale )}" id="change_img_desc" class="form-horizontal">
                            ...
                            <label class="number">
                                <span class="m-r_10" style="margin-right: 51px;" >{lang("Position", 'gallery')}:</span>
                                <input type="text" value="{$image.position}" name="position" class="pull-right" style="width: 500px; margin-bottom: 10px"/>
                            </label>

                            <label>
                                <span class="m-r_10">{lang("Title", 'gallery')}:</span>
                                <input type="text" class="textbox_short pull-right" name="title" value="{$image.title}" style="width: 500px;"/>
                            </label>
                        ...
                        <form method="post" action="{site_url('admin/components/cp/gallery/rename_image/' . $image.id)}" id="change_imn_name_form">
                            ...
                            <button type="submit" class="btn btn-primary formSubmit" data-form="#change_imn_name_form">
                                <i class="icon-ok"></i> {lang("Save", 'gallery')}</button>
                            {form_csrf()}
                        </form>
                        { */ }
